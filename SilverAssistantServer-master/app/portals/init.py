...
def verify_code(user_id, code):
    try:
        ...
        if user.verification_code == str(code):
            ...
            r1 = models.UserUserAssociation(user_id1=user.ID, user_id2=app.config['SILVER_ASSISTANT_SYSTEM_USER_ID'], status='confirmed')
            r2 = models.UserUserAssociation(user_id2=user.ID, user_id1=app.config['SILVER_ASSISTANT_SYSTEM_USER_ID'], status='confirmed')
            db.session.add(r1)
            db.session.add(r2)
            db.session.commit()
